...
1. The `stackgen.yml` file. The contents allowed inside it can be found [here](Documentation/Reference/Struct/StackGenFile.md). Examples of it can be found [here](Examples/swift/stackgen.yml) and [here](stackgen.yml).
2. A bunch of templates. At the moment the only supported template language is [stencil](https://github.com/stencilproject/Stencil). StackGen comes with several groups of templates out of the box, which you can use as is or as a reference to create custom ones: find them [here](Templates). The context that is available from inside the templates is defined [here](Documentation/Reference/Struct/Context.Output.md). In addition, there are several custom stencil filters available: [abs](Documentation/Reference/Classes/StencilFilter.Absolut.md), [expand](Documentation/Reference/Classes/StencilFilter.ExpandDependencies.md), [pathExists](Documentation/Reference/Classes/StencilFilter.PathExists.md), [rm](Documentation/Reference/Classes/StencilFilter.RelativeToModule.md) and [rr](Documentation/Reference/Classes/StencilFilter.RelativeToRoot.md). Note that if the resulting file after rendering a template is empty, then the file will not be created.
